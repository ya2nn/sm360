...
const FieldText = ({
  ...
  if (field) {
    ...
    if (field.field.name.includes('date')) _type = 'text'
    else if (field.field.name.includes('postalcode')) _type = 'text'
    else if (field.field.name.includes('phone')) _type = 'tel'
    else if (field.field.name.includes('email')) _type = 'email'
  }
  ...
  const handlePlaceholderBlur = () => {
    if (inputRef.current.value.length > 0)
      labelRef.current.classList.remove('hidden')
    else labelRef.current.classList.add('hidden')
  ...
  return (
    <p className={`a-fieldText relative mb-[5px] ${extraClasses}`}>
      <label
        ref={labelRef}
        htmlFor={`${_field_name}${
          customFieldId ? '-'.concat(customFieldId) : ''
        }`}
        className={`a-fieldText__label absolute ${
          !defaultFieldValue ? 'hidden' : ''
        } left-[15px] top-[5px] flex ${labelExtraClasses}`}
      >
        <span className='text-[11px] leading-none'>{_field_label}</span>
      </label>
      <input
        ref={inputRef}
        type={_type}
