 ...
 * @return { Object }
 */
export const extractUrlParametersAsObjectWithArrayProperties = (
  url = window.location
) => {
  const urlParameters = new URLSearchParams(url.search)
  ...
  for (const key in parametersObject) {
    ...
    const valueAsArray = parametersObject[key].indexOf(',')
      ? parametersObject[key].split(',')
      : parametersObject[key].split()

    // Transform the numbers back to an integer type
    const valueAsArrayTyped = valueAsArray.map((item) =>
      isNaN(item) ? item : parseInt(item)
    )

    // Assign value back to object's property
