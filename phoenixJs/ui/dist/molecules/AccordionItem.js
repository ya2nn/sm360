...
const AccordionItem = ({
  ...
  const disabledClasses = disabled ? '-disabled text-gray-400 cursor-not-allowed' : '';
  const positionContentContainerClasses = position === 'horizontal' ? 'absolute left-0 w-full' : '';
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: `m-accordionItem ${openTriggerClasses} ${extraClasses}`
  }, /*#__PURE__*/React.createElement("button", {
    className: `m-accordionItem__title flex w-full items-center justify-between space-x-4 whitespace-nowrap rounded bg-white p-[15px] text-left ${isOpen ? 'text-primary' : ''} ${disabledClasses} ${buttonExtraClasses} ${_buttonActiveExtraClasses}`,
    ...
    symbolId: "arrow-down",
    extraClasses: `w-[15px] h-[15px] transition-transform ${isOpen ? 'rotate-180' : ''}`
  }))), /*#__PURE__*/React.createElement("div", {
    className: `transition-all duration-300 ${!isOpen ? 'hidden' : ''} ${positionContentContainerClasses} ${contentContainerExtraClasses}`,
    role: "region",
